...
class YahtzeeBonusProbabilities:
    ...
    def __init__(self):
        ...
        #self.probabilities = np.array(range(1,6))      
    ...
    def generate_states(self):
        self.all_states = np.array(list(itertools.product(range(0, self.n_dice_sides), repeat=self.n_categories))) #range(-1, self.n_dice_sides)
        for i, state in enumerate(self.all_states):
            if 0 in state:
                #Check if state can reach goal state
                proof_state = state.copy()
                proof_state[proof_state == 0] = self.n_dice_sides
    ...
    def allowed_transitions(self, r, specific_state):
        ...
        keep_dice_indices = np.where(np.array(specific_state) != 0)[0]
        ...
        variable_dice_indices = np.where(np.array(specific_state) == 0)[0]    
        #Filter the relevant dice values from all states
        ...
        filtered_variable_states_indices = np.where((filtered_variable_dice != 0).sum(axis=1) == 1)[0]
        ...
        #Normalize the row
        values = values/np.sum(values) #Experimental Comment In
    ...
    def calc_initial_probabilities(self):
        ...
        for _ in range(self.n_categories + 7):
            start = time.time()
